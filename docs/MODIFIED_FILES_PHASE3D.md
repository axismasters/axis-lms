...
이미지와 지시를 준 케이스). 수정 파일 3개.

### 로그인 히어로 이미지 교체 — 중앙정렬 개선판

사용자가 "로그인 화면에 마크가 중앙정렬이 안되어 보임"이라며 새 이미지를
제공했다. 기존 이미지(623×274)와 비교해보니, 기존 이미지는 우상단에
"AXIS MATH ACADEMY" 라벨이 있어 구성이 비대칭이었고(그 라벨의 무게 때문에
전체 캔버스를 꽉 채워 렌더링했을 때 마크가 왼쪽으로 치우쳐 보임), 새 이미지
(534×291)는 그 라벨이 빠지고 마크+태그라인+ANALYSIS·PRECISION·RESULT만
남아 훨씬 대칭적으로 중앙정렬되어 있었다.

| 파일 | 수정 내용 |
|---|---|
| `src/assets/brand/axis-hero-dark.png` | 사용자가 새로 제공한 중앙정렬 개선판 이미지(534×291)로 교체. `LoginPage.tsx` 코드는 무변경(같은 파일명, `w-full` 반응형 렌더링이라 비율이 달라져도 별도 코드 수정 불필요). |

### 관리자 사이드바 네이비 색상 최신 이미지 기준 재확정

사용자가 "사이드바 네이비도 이 색상으로" 요청했다. 새 이미지에서 다시
정밀 샘플링한 결과 `#040E1F`(가중평균)이 나왔는데, 기존 토큰값(`#040D1E`,
v3-r9-r2에서 이전 이미지 세트로 산출)과 RGB 유클리드 거리 1.41 — 사람 눈이
구분 가능한 최소 임계값(통상 5~10 이상)에 한참 못 미치는, 사실상 동일한
색이다(같은 브랜드 이미지 세트에서 나온 값이니 당연한 결과). 그래도 지시대로
최신 이미지 기준으로 토큰을 재확정했다.

| 파일 | 수정 내용 |
|---|---|
| `src/index.css` | `--brand-navy` 토큰을 `oklch(0.1644 0.0397 257.96)`(← `#040E1F`)로 갱신. `.axis-sidebar`는 v3-r9-r3에서 이미 `var(--brand-navy)`를 참조하도록 고쳐뒀기 때문에, 이 한 줄만으로 사이드바를 포함한 모든 토큰 기반 요소에 자동 반영됐다. |
| `src/lib/brandColors.ts` | `AXIS_NAVY`/`AXIS_NAVY_OKLCH` 상수를 동일 값으로 갱신. |

**의도적으로 안 한 것**: v3-r9-r2에서 전역 치환했던 67개 파일의 하드코딩
`#040D1E` 리터럴은 이번엔 재치환하지 않았다 — 새 값과의 색차가 RGB 거리
1.41로 사실상 무의미한 수준이라, 300개 넘는 위치를 다시 건드리는 리스크가
얻는 이득보다 크다고 판단했다(상세 근거는 `QA_PHASE3D.md` v3-r9-r4 섹션 참조).


**베이스라인**: v3-r8 + v3-r9-r1/r9-r2에서 유지 지시된 부분(로그인 히어로 이미지,
실제 마크, IF 엔진 단일 진입점). 수정 파일 1개.

### 원인: v3-r9-r2의 전역 치환이 `.css` 파일을 빠뜨렸다

v3-r9-r2에서 `#081F4D` 리터럴을 전역 치환할 때 Python 스크립트가
`glob("src/**/*.tsx")`와 `glob("src/**/*.ts")`만 대상으로 했다 — `.css` 파일은
애초에 검색 대상에 없었다. 그 결과 `src/index.css`의 `.axis-sidebar` 규칙
안에 있던 `background-color: #081F4D;` 한 줄이 갱신되지 않고 남아있었다.
사이드바는 앱에서 가장 넓은 면적을 차지하는 네이비 요소라, 이 한 줄이 남은
채로는 "이미지 기준 네이비로 통일했다"는 이전 라운드의 주장이 사실상 깨져
있었던 셈이다.

| 파일 | 수정 내용 |
|---|---|
| `src/index.css` | `.axis-sidebar`의 `background-color: #081F4D;`를 `background-color: var(--brand-navy);`로 교체(지시받은 그대로). 리터럴이 아니라 토큰 참조로 바꿔서, 앞으로 `--brand-navy` 값이 다시 바뀌어도 이 줄이 자동으로 따라가고 같은 문제가 재발하지 않도록 했다. |

### 재검증

`grep -rn "#081F4D" src`(확장자 제한 없이 `src/` 전체)를 다시 실행해 실제 UI
코드에 남은 하드코딩이 0건임을 확인했다(주석 3곳만 남음 — 전부 변경 이력을
설명하는 문서성 주석이라 지시 기준상 정리 대상 아님). IF 엔진 단일 진입점,
로그인 히어로 이미지, 실제 마크 이미지는 이번에 전혀 건드리지 않았다.
