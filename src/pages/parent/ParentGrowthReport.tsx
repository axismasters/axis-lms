  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 64 }} preserveAspectRatio="none">
        <polyline points={coords.map(c => `${c.x},${c.y}`).join(' ')} fill="none" stroke="oklch(0.511 0.262 276.966)" strokeWidth="1.5" vectorEffect="non-scaling-stroke" />
        {coords.map((c, i) => (
          ...
          {(avgPct !== null || result.highestScore != null) && (
            <div>
              ...
              <div className="space-y-2">
                <HBar label="내 점수" value={result.earnedScore} max={result.totalPoints} color="oklch(0.511 0.262 276.966)" valueLabel={`${result.earnedScore}점`} />
      ...
      <div className="max-w-3xl mx-auto px-4 py-5 space-y-4">
        ...
        <div className="axis-card p-5">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-xl font-bold text-white" style={{ background: 'oklch(0.511 0.262 276.966)' }}>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="rounded-lg p-3 text-center" style={{ background: 'oklch(0.96 0.004 250)' }}>
              <div className="font-black text-base tabular-nums" style={{ color: 'oklch(0.511 0.262 276.966)' }}>{allPublishedResults.length}건</div>
          ...
          <div className="flex items-center gap-1.5">
            {([{ k: 'month' as Period, l: '이번 달' }, { k: '3months' as Period, l: '최근 3개월' }, { k: 'all' as Period, l: '전체' }]).map(p => (
              <button key={p.k} onClick={() => setPeriod(p.k)}
                ...
                style={period === p.k
                  ? { background: 'oklch(0.511 0.262 276.966)', color: 'white' }
            ...
            {ifSummary.totalRecordsAnalyzed > 0 && (
              <div className="axis-card p-4">
                ...
                <p className="text-sm mb-3" style={{ color: 'oklch(0.35 0.02 250)' }}>
                  ...
                  약 <strong style={{ color: 'oklch(0.511 0.262 276.966)' }}>+{ifSummary.avgImprovementPct ?? 0}%p</strong> 향상 여지가 있습니다.
                </p>
        ...
        {tab === 'university' && (
          <Link href="/parent/target-summary" style={{ display: 'block' }}>
            <div className="axis-card axis-card-clickable p-5">
              ...
              <div className="flex items-center gap-1 mt-3 text-xs font-medium" style={{ color: 'oklch(0.511 0.262 276.966)' }}>
          ...
          <>
            ...
            <div className="axis-card p-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="rounded-lg p-3 text-center" style={{ background: 'oklch(0.96 0.004 250)' }}>
                  <div className="font-black text-base tabular-nums" style={{ color: 'oklch(0.511 0.262 276.966)' }}>{monthlyHomeworkRate !== null ? `${monthlyHomeworkRate}%` : '-'}</div>
