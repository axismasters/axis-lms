        ...
        <div className="flex items-center gap-2">
          {!needsPublishAction ? (
            ...
            <>
              ...
              {canPublishPerm && (
                <Button size="sm" onClick={() => setPublishConfirmOpen(true)} className="h-8 text-xs gap-1.5" style={{ background: '#040D1E' }}>
            ...
            <Button
              ...
              style={{ background: canPublish(exam.id) ? '#040D1E' : undefined }}
        ...
        <div className="flex border-b" style={{ borderColor: 'oklch(0.92 0.005 250)' }}>
          {TABS.map((t) => (
            <button
              ...
              style={{ borderColor: tab === t.key ? '#040D1E' : 'transparent', color: tab === t.key ? '#040D1E' : 'oklch(0.55 0.015 250)' }}
            >
              {t.icon} {t.label}
            </button>
          ))}
      ...
      <Dialog open={publishConfirmOpen} onOpenChange={(o) => !o && setPublishConfirmOpen(false)}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button size="sm" onClick={handlePublish} className="h-8 text-xs gap-1" style={{ background: '#040D1E' }}>
            ...
            {submissions.map((sub) => {
              ...
              return (
                <tr key={sub.id} className="axis-table-row border-b" style={{ borderColor: 'oklch(0.95 0.003 250)' }}>
                  ...
                  <td className="px-3 py-2 text-center text-xs font-semibold tabular-nums" style={{ color: '#040D1E' }}>
      ...
      <Dialog open={!!correctionModal} onOpenChange={(o) => !o && setCorrectionModal(null)}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button size="sm" onClick={handleCorrectionSave} className="h-8 text-xs" style={{ background: '#040D1E' }}>정정 저장</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
