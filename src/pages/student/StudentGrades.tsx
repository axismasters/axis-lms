// AXIS LMS v1.2 - StudentGrades (Student Portal Foundation v1)
// 학생 전용 성적 조회 — 공개/반영된 결과만 표시. 읽기 전용.
// 정책:
//   - 반 단위 시험: totalScore 확정(채점완료) 시 노출
//   - 학원 전체 시험: publishedAt 있을 때만 노출
//   - 결석/미채점은 절대 표시 안함 (getPublishedResultsForStudent 정책 적용)
        ...
        {results.length === 0 ? (
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { label: '응시 시험', value: `${results.length}회`, color: 'oklch(0.511 0.262 276.966)' },
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs mt-0.5" style={{ color: 'oklch(0.55 0.015 250)' }}>
                          ...
                          {!r.classId && (
                            <span className="ml-1 px-1.5 py-0.5 rounded text-xs" style={{ background: 'oklch(0.95 0.04 250)', color: 'oklch(0.4 0.15 250)' }}>
                              전체
                            </span>
                          )}
                        </div>
