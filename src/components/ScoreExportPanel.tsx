...
export default function ScoreExportPanel({ scopeLabel, availableExams, students, classes, submissions }: ScoreExportPanelProps) {
  ...
  const chipStyle = (active: boolean) => ({
    background: active ? 'oklch(0.511 0.262 276.966)' : 'oklch(0.96 0.004 250)',
    color: active ? 'white' : 'oklch(0.4 0.015 250)',
    border: active ? '1px solid oklch(0.511 0.262 276.966)' : '1px solid oklch(0.9 0.008 250)',
  });

  return (
    <div className="space-y-4">
      <div className="axis-card p-4 text-xs" style={{ borderLeft: '3px solid oklch(0.511 0.262 276.966)', color: 'oklch(0.5 0.015 250)' }}>
          ...
          <div className="space-y-1 max-h-64 overflow-y-auto pr-1">
            {sortedExams.map((exam) => {
              ...
              return (
                ...
                >
                  {active ? <CheckSquare size={14} style={{ color: 'oklch(0.511 0.262 276.966)' }} /> : <Square size={14} style={{ color: 'oklch(0.75 0.01 250)' }} />}
                ...
                <tbody>
                  {rows.map((r) => (
                    <tr key={`${r.examId}-${r.studentId}`} className="axis-table-row border-b" style={{ borderColor: 'oklch(0.95 0.003 250)' }}>
                      ...
                      <td className="px-2.5 py-1.5 text-right tabular-nums font-semibold" style={{ color: 'oklch(0.511 0.262 276.966)' }}>{r.percentage}%</td>
